...
assert.match(source, /(-Encoding\s+(ascii|utf8NoBOM)|ASCIIEncoding|UTF8Encoding)/i);
assert.match(source, /WriteAllText/);
assert.doesNotMatch(source, /\$\{name\}/);
assert.doesNotMatch(source, /\$\{text\.replace/);
assert.doesNotMatch(source, /-Encoding\s+UTF8\b/i);
assert.doesNotMatch(source, /Set-Content\s+-LiteralPath\s+\$LocalRemoteScript/i);
